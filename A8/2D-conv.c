...
int conv_column(int *sub_grid, int i, int nrows, int DIM, int *kernel,
                int kernel_dim) {
  int counter = 0;
  ...
  counter = counter +
            sub_grid[i] * kernel[(((kernel_dim - 1) * (kernel_dim + 1)) / 2)];
...
int conv(int *sub_grid, int i, int nrows, int DIM, int *kernel,
  ...
  for (int j = 1; j < (num_pads + 1); j++) {
    ...
    if (i + j - end <= 0) {  // if column is valid
      counter = counter +
                conv_column(sub_grid, i + j, nrows, DIM, kernel, kernel_dim);
    }
...
int main(int argc, char **argv) {
  ...
  for (iters = 0; iters < num_iterations; iters++) {
    ...
    if (num_procs > 1) {
      if (ID % 2 == 1) {
        MPI_Recv(pad_row_lower, DIM * num_pads, MPI_INT, next, 1,
                 MPI_COMM_WORLD, &status);
        MPI_Recv(pad_row_upper, DIM * num_pads, MPI_INT, prev, 1,
                 MPI_COMM_WORLD, &status);
      } else {
      ...
      if (ID % 2 == 1) {
        ...
        MPI_Recv(pad_row_lower, DIM * num_pads, MPI_INT, next, 0,
                 MPI_COMM_WORLD, &status);
        MPI_Recv(pad_row_upper, DIM * num_pads, MPI_INT, prev, 0,
                 MPI_COMM_WORLD, &status);
      }
    } else {
      pad_row_lower = upper;
    ...
    memcpy(&sub_grid[DIM * num_pads], &main_grid[DIM * start],
           sizeof(int) * DIM * nrows);
    memcpy(&sub_grid[DIM * (nrows + num_pads)], pad_row_lower,
           sizeof(int) * DIM * num_pads);
    ...
    if (ID != 0) {
      ...
      MPI_Recv(&main_grid[0], DIM * DIM, MPI_INT, 0, 10, MPI_COMM_WORLD,
               &status);
    } else {
      ...
      for (int k = 1; k < num_procs; k++) {
        MPI_Recv(&main_grid[DIM * (DIM / num_procs) * k], nrows * DIM, MPI_INT,
                 k, 11, MPI_COMM_WORLD, &status);
      }
